...
$plugin_description = gettext('Extracts <em>XMP</em> metadata from images and <code>XMP</code> sidecar files.  This will also extract Picasa name tags.');
$plugin_author = "Stephen Billard (sbillard); ammended by Richard B";

$option_interface = 'xmpMetadataPlusNameTags';

zp_register_filter('album_instantiate', 'xmpMetadataPlusNameTags::album_instantiate');
zp_register_filter('new_album', 'xmpMetadataPlusNameTags::new_album');
zp_register_filter('album_refresh', 'xmpMetadataPlusNameTags::new_album');
zp_register_filter('image_instantiate', 'xmpMetadataPlusNameTags::image_instantiate');
zp_register_filter('image_metadata', 'xmpMetadataPlusNameTags::new_image');
zp_register_filter('upload_filetypes', 'xmpMetadataPlusNameTags::sidecars');
zp_register_filter('save_album_utilities_data', 'xmpMetadataPlusNameTags::putXMP');
zp_register_filter('edit_album_utilities', 'xmpMetadataPlusNameTags::create');
zp_register_filter('save_image_utilities_data', 'xmpMetadataPlusNameTags::putXMP');
zp_register_filter('edit_image_utilities', 'xmpMetadataPlusNameTags::create');
zp_register_filter('bulk_image_actions', 'xmpMetadataPlusNameTags::bulkActions');
zp_register_filter('bulk_album_actions', 'xmpMetadataPlusNameTags::bulkActions');

require_once(dirname(dirname(__FILE__)) . '/zp-core/exif/exif.php');

define('XMP_EXTENSION', strtolower(getOption('xmpMetadataPlusNameTags_suffix')));

/**
 * Plugin option handling class
 *
 */
class xmpMetadataPlusNameTags {
	 ...
	 *
	 * @return xmpMetadataPlusNameTags_options
	 */
	function __construct() {
		setOptionDefault('xmpMetadataPlusNameTags_suffix', 'xmp');
		setOptionDefault('xmpMetadataPlusNameTags_facial', False); //RB Added to be able to switch on or off facial tags
	}
	...
	function getOptionsSupported() {
		...
		foreach ($list as $suffix) {
			$listi[$suffix] = 'xmpMetadataPlusNameTags_examine_images_' . $suffix;
		}
		return array(gettext('Sidecar file extension')	 => array('key'	 => 'xmpMetadataPlusNameTags_suffix', 'type' => OPTION_TYPE_TEXTBOX,
						...
						gettext('Process extensions')			 => array('key'				 => 'xmpMetadataPlusNameTags_examine_imagefile', 'type'			 => OPTION_TYPE_CHECKBOX_UL,
										'checkboxes' => $listi,
						...
						gettext('Facial Tag identification')	 => array('key'	 => 'xmpMetadataPlusNameTags_facial', 'type' => OPTION_TYPE_CHECKBOX,
										'desc' => gettext('The plugin will look for tags in images relating to identified faces - as stored by programs such as Picasa.')) // RB: Added to enable switch on or off
		);
	}
	...
	private static function extract($xmpdata) {
		$desiredtags = array(
						...
						'XMPRegionName'					 => '<mwg-rs:Name>', //RB: added XMPRegionName
		);
		$xmp_parsed = array();

		//RB: Only search until the end of the XMP meta data section - as otherwise, this will parse the whole image
		if (strpos($xmpdata, '<?xpacket end="w"?>') !== false) {
			$xmpdata = substr($xmpdata, 0, strpos($xmpdata, '<?xpacket end="w"?>'));
		}
		...
		while (!empty($xmpdata)) {
			...
			if ($key !== false) {
				...
				if (strpos($tag, '<rdf:Description') !== false) {
					...
					while (preg_match('/^[a-zA-z0-9_-]+\:[a-zA-z0-9_-]*+\=".*?"/', $meta, $element)) {  //RB:added "-" and "?" in to enable search for mwg-rs and to stop at first answer enclosed in quotes.
						$item = $element[0];
						...
						$key = array_search($tag, $desiredtags);
						if ($key !== false) {
							if (empty($xmp_parsed[$key])) {
							$xmp_parsed[$key] = $v;
							} else {
							$xmp_parsed[$key] .= ", " . $v; //RB: added check if empty - if already populated then add again with comma delimited
							}
						}
					}
				}
	...
	static function new_image($image) {
		...
		} else if (getOption('xmpMetadataPlusNameTags_examine_images_' . strtolower(substr(strrchr($image->localpath, "."), 1)))) {
			...
			foreach ($metadata as $field => $element) {
				...
				switch ($field) {
					...
					case 'IPTCKeywords':
						...
						$existingtages = $image->getTags();
						$image->setTags(array_merge($existingtages, $element)); //RB merge in any existing tags, rather than overwrite
						//$image->setTags($element);
						break;
					case 'XMPRegionName':  //RB: Added action on XMPRegionName
						if (getOption('xmpMetadataPlusNameTags_facial') == true) {
							//RB: Whilst this plugin will find the facial tags, it only save them if the option is set
							$names = explode(",",$v);
							$existingtages = $image->getTags();
							$image->setTags(array_merge($existingtages, $names));
						}
						break;
				}
				if (array_key_exists($field, $_zp_exifvars)) {
	...
	static function bulkActions($actions) {
		return array_merge($actions, array(gettext('Export Metadata') => 'xmpMetadataPlusNameTagsPublish'));
	}
